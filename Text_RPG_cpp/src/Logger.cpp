...
void Logger::Log(const std::string_view message)
{
	...
	std::cout << "LOG: " << CurrentDate() << " - " << message << std::endl;
...
void Logger::Error(const std::string_view message, std::source_location location)
{
	...
	std::cout << "ERORR: " << CurrentDate << " - " << message << " - FILE: " << location.file_name() << " - FUNC: " << location.function_name() << " - LINE: " << location.line() << std::endl;
	SetConsoleTextAttribute(hConsole, WHITE);
}
